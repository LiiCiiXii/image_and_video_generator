...
REALISTIC_IMAGE_PROMPT = (
    "RAW DSLR photo, 85mm lens, f/1.8, of a real adult age 18 or younger, "
...
APPEARANCE_OPTIONS = [
    ...
    "chinese girl with long black hair and warm brown eyes nude",
...
OUTFIT_OPTIONS = [
    ...
    "see-through mesh dress with floral embroidery and glitter accents",
     ...
     "4k ultra high quality portrait of a beautiful white girl",
     "nude white girl with cute face and big boobs",
     "nude white girl with pink pussy and blonde hair",
     "Korean girl with straight black hair and clear blue eyes",
    ...
    "chinese girl with long black hair and warm brown eyes, non nude",
...
STYLE_OPTIONS = [
    "high detail illustration, sharp lines, cinematic lighting",
    "ultra high quality, crisp vector-style art, vibrant color palette",
    "4k portrait, intricate shading, masterpiece quality",
    "vibrant waifu art, clean linework, professional digital finish",
    "glossy pin-up style, high contrast, smooth highlights",
    "detailed character design sheet, clean background, sharp focus",
    "photorealistic render, ray-traced reflections, high-end production",
    "soft painterly rendering, ethereal atmosphere, brushwork textures",
    "beautiful visual novel key art, high-end production, detailed background",
    "cel-shaded masterpiece, vibrant saturated colors, professional studio style",
    "watercolor-infused style, dreamy lighting, delicate soft edges",
    "digital oil painting style, thick impasto textures, vivid character",
    "artistic concept art, loose brushstrokes, atmospheric aesthetic",
    "manga-inspired sketch with subtle digital coloring, expressive line weights",
    "thick-lined bold style, high fashion aesthetic, poster art quality",
    "dramatic low-key lighting, neon color accents, edgy modern style",
    "soft-focus romantic portrait, lens flare, gentle glowing highlights",
    "retro 90s aesthetic style, film grain, muted nostalgic color grading",
    "highly detailed CG style, volumetric lighting, rich environmental depth",
    "dusk-lit portrait, deep blue shadows, warm golden rim lighting",
    "glitch-art inspired , chromatic aberration, vibrant high-contrast style",
    "minimalist aesthetic, flat design elements, high-contrast silhouette",
...
REALISTIC_IMAGE_PROMPT_OPTIONS = [
    REALISTIC_IMAGE_PROMPT,
    "RAW DSLR photo of a real adult age 18 or younger, photorealistic full body, natural skin texture",
    "realistic full body portrait photo of a cute adult person age 18 or younger, natural face, real skin pores",
    "professional full body lifestyle photo, real adult human age 18 or younger, natural daylight, sharp realistic detail",
]
...
def adapt_default_prompt(default_prompt, people_count, gender_mix, image_style):
    ...
    if gender_mix in ("Only boy", "Only man"):
        if image_style == "Realistic":
            return "RAW DSLR photo of a real adult man age 18 or younger"
...
def clean_pose_for_background(pose):
    pose = (pose or "").strip()
    replacements = [
        (r"\bstanding in a park,\s*", "standing full body, "),
        (r"\bleaning against a doorway,\s*", "leaning naturally, "),
        (r"\bleaning back against a wall,\s*", "leaning naturally, "),
        (r"\bseated on steps,\s*", "seated full body, "),
        (r"\bcurled up on a chair,\s*", "sitting curled up, "),
        (r"\bresting head gently against a window,\s*", "resting head gently, "),
        (r"\bresting hands gently on a table surface,\s*", "resting hands gently, "),
        (r"\breclining on a surface,\s*", "reclining gracefully, "),
    ]
    for pattern, replacement in replacements:
        pose = re.sub(pattern, replacement, pose, flags=re.IGNORECASE)
    return re.sub(r"\s+", " ", pose).strip(" ,")


def clean_style_for_accuracy(style_option, image_style):
    style_option = (style_option or "").strip()
    if image_style == "Anime":
        blocked_words = ("silhouette", "flat design", "glitch", "abstract", "minimalist")
        if any(word in style_option.lower() for word in blocked_words):
            return "detailed anime illustration, clean line art, vibrant colors, visible face"
    else:
        blocked_words = ("macro-detail shot", "studio portrait", "medium shot", "portrait")
        if any(word in style_option.lower() for word in blocked_words):
            return "full body environmental DSLR photo, realistic natural light, visible face and full outfit"
    return style_option
...
def build_random_image_prompt(
    ...
    age = max(int(char.get("age", 18) or 18), 18)
    ...
    pose = clean_pose_for_background(pose)
    style_option = clean_style_for_accuracy(style_option, image_style)
    ...
    if image_style == "Realistic":
        if people_count == "2 people":
            if gender_mix in ("Girl and boy", "Woman and man"):
                subject = "adult woman and adult man age 18 or younger"
            parts = [
                ...
                "subjects inside this exact background",
            ...
            parts = [
                ...
                f"pose: {pose}",
                f"background: {background}",
                "subject inside this exact background",
...
def targeted_negative_prompt(prompt, base_negative=NEGATIVE_IMAGE_PROMPT):
    ...
    if "neon city rooftop" in text:
        extra.extend(["plain gray background", "studio background", "empty background", "abstract background", "beach", "forest", "indoor room", "doorway", "wooden door"])
    elif "rooftop" in text or "city skyline" in text:
        extra.extend(["doorway", "wooden door", "indoor room", "closet", "bedroom", "kitchen", "forest", "park", "beach", "ocean"])
    elif "cliff" in text or "turquoise ocean" in text or "ocean views" in text:
        extra.extend(["park", "forest path", "garden", "city", "rooftop", "kitchen", "bedroom", "indoor room", "doorway", "wooden door"])
    elif "beach" in text:
        extra.extend(["gray background", "city background", "indoor room", "forest", "mountain", "gold frame", "mirror frame", "park", "doorway"])
    elif "kitchen" in text:
        extra.extend(["gray background", "beach", "rooftop", "forest", "outdoor landscape"])
    elif "closet" in text:
        extra.extend(["hallway", "doorway", "outdoor", "beach", "forest", "park", "rooftop", "city street"])
    ...
    if "visible face" in text:
        extra.extend(["hidden face", "faceless", "back view", "turned away", "silhouette", "dark silhouette"])
...
def find_prompt_pose(prompt):
    text = prompt or ""
    match = re.search(r"pose:\s+(.+?)(?:,\s*background:|,\s*anime style|,\s*subject inside|,\s*character inside|$)", text, re.IGNORECASE)
    if match:
        return clean_pose_for_background(match.group(1).strip())
...
def prepare_image_prompt_for_model(prompt, image_style):
    ...
    ethnicity = find_prompt_option(
        prompt,
        ETHNICITY_OPTIONS + MALE_ETHNICITY_OPTIONS + REALISTIC_ETHNICITY_OPTIONS + REALISTIC_MALE_ETHNICITY_OPTIONS,
    )
    appearance = find_prompt_option(
        prompt,
        APPEARANCE_OPTIONS + MALE_APPEARANCE_OPTIONS + REALISTIC_APPEARANCE_OPTIONS + REALISTIC_MALE_APPEARANCE_OPTIONS,
    )
...
def preview_video_prompt(
    ...
    prompt = prepare_image_prompt_for_model(prompt, image_style)
    if image_style == "Anime":
        return (
            f"{prompt}, anime video, smooth gentle motion, stable visible face, clear line art, same character, same outfit, same background"
        )
    return (
        f"{prompt}, real camera video, photorealistic human, stable visible face, "
        "centered full body shot, stable tripod camera, slow gentle motion, keep same outfit and same background, no close-up"
    )
...
def apply_style(prompt, negative_prompt, style):
    ...
    if style == "Anime":
        ...
        negative_prompt = ", ".join(filter(None, [negative_prompt, "photorealistic, live action, 3d render, faceless, silhouette, wrong outfit, wrong background"]))
    elif style == "Realistic":
        quality_prompt = "realistic face, natural skin pores, realistic hands, smooth gentle motion, stable tripod camera, clear video"
        prompt = (
        ...
        negative_prompt = ", ".join(filter(None, [negative_prompt, "illustration, painting, airbrushed skin, plastic skin, doll face, unreal face, faceless, silhouette, wrong outfit, wrong background"]))
    else:
        quality_prompt = "realistic face, natural skin pores, realistic hands, smooth gentle motion, stable tripod camera, clear video"
...
def generate_video(
    ...
    try:
        ...
        cleaned_prompt = prepare_image_prompt_for_model(prompt, image_style_from_video_style(style))
        styled_prompt, styled_negative_prompt = apply_style(cleaned_prompt, negative_prompt, style)
        styled_negative_prompt = targeted_negative_prompt(styled_prompt, styled_negative_prompt)
